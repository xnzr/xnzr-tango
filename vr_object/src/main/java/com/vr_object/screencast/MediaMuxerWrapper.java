...
public class MediaMuxerWrapper {
	...
	public MediaMuxerWrapper(final Context context, final String _ext) throws IOException {
		...
		try {
			File dir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES), "xnzr-rec");
			if (!dir.exists()) {
				...
				if (!res) {
					...
					Toast.makeText(context, context.getString(R.string.no_sdcard_permission), Toast.LENGTH_LONG).show();
				}
			}
            mOutputPath = dir.getAbsolutePath() + "/rec_" + getDateTimeString() + ext;
